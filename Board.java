...
/*private class BoardState{
    ...
    //useful masks for different types of squares on the board
    long maskValid = 0b0000011110111111110111111110111111110111100000L; //all the valid board squares (the 0s are padding)
    long maskBlackBack = 0b0000011110000000000000000000000000000000000000L; //the home row squares for black
    long maskWhiteBack = 0b0000000000000000000000000000000000000111100000L; //the home row squares for white
    long maskCenter = 0b0000000000000000000011001100000000000000000000L; //the central 8 squares (4 playable squares)

    //the 3 base bitboards used to permanently represent the board:
    ...
    //a board with no pieces on, the players return this as their best move if they cannot play
    void nullBoard(){
        blackPieces = 0b0000000000000000000000000000000000000000000000L;
        whitePieces = 0b0000000000000000000000000000000000000000000000L;
        kings =       0b0000000000000000000000000000000000000000000000L;
    ...
    long emptySquares() { return ~blackPieces & ~whitePieces & maskValid; } //empty squares (valid squares not black nor white)

    //methods to query the board
    boolean isBlack(int position){
        return ((findMask(45-position) & blackPieces) != 0);
    } //returns true if a given position has a black piece
    boolean isWhite(int position){
        return ((findMask(45-position) & whitePieces) != 0);
    } //returns true if a given position has a white piece
